<%@ Page Title="" Language="C#" MasterPageFile="//MasterPage/MasterPageCompzit.master" AutoEventWireup="true" CodeFile="gen_Insurance_Provider_List.aspx.cs" Inherits="WMS_Wms_Master_gen_Insurance_Provider_gen_Insurance_Provider_List" %>
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" Runat="Server">
    ...
    <script type="text/javascript">
        ...
        function DuplicationName() {
            $("#divWarning").html("Recalling denied!. Insurance provider name can’t be duplicated.");
        ...
        function SuccessCancelation() {
            $("#success-alert").html("Insurance provider cancelled successfully.");
            $("#success-alert").fadeTo(2000, 500).slideUp(500, function () {
            });
        }
        function SuccessRecall() {
            $("#success-alert").html("Insurance provider recalled successfully.");
            $("#success-alert").fadeTo(2000, 500).slideUp(500, function () {
            });
        }
        ...
        function AlreadyCncl() {
            $("#divWarning").html("Sorry, Cancellation denied. This entry is already cancelled!");
            $("#divWarning").fadeTo(3000, 500).slideUp(500, function () {
            });
            return false;
        }
     ...
     <script>
         ...
         function OpenCancelView(StrId) {
             ezBSAlert({
                 type: "confirm",
                 messageText: "Do you want to cancel this insurance provider?",
                 alertType: "info"
             }).done(function (e) {
                 if (e == true) {
            ...
            $.ajax({
                ...
                url: "gen_Insurance_provider_List.aspx/ReOpenConfByID",
                data: '{orgID: "' + orgID + '",corptID: "' + corptID + '",userID: "' + userID + '",MasterDbId: "' + MasterDbId + '",Mode: "' + Mode + '",reasonmust: "' + reasonmust + '",cnclRsn: "' + cnclRsn + '"}',
                dataType: "json",
                success: function (data) {
                    window.location.href = "gen_Insurance_provider_List.aspx?InsUpd=" + data.d;
                }
            });
            return false;
        }
        function SuccessSts() {
            $("#success-alert").html("Insurance provider status changed successfully.");
            $("#success-alert").fadeTo(2000, 500).slideUp(500, function () {
            });
            return false;
        }
        ...
        function ReCallAlert(StrId) {
            ezBSAlert({
                type: "confirm",
                messageText: "Do you want to recall this insurance provider?",
                alertType: "info"
            }).done(function (e) {
                if (e == true) {
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphMain" Runat="Server">
    ...
    <div class="content_sec2 cont_contr">
      ...
      <div class="content_box1 cont_contr">
        <h1 class="h1_con">Insurance Provider</h1>


            <div class="form-group fg2" id="divBnk">
          <label for="email" class="fg2_la1">Insurance Type:<span class="spn1"></span></label>
          <select class="form-control fg2_inp1" id="ddlProviderType" runat="server">
          </select>           
        </div>
 ...
 <a href="#" type="button" class="print_o" title="Print page" onclick="return PrintClick();">
  <i class="fa fa-print"></i>
    ...
    <a href="gen_Insurance_Provider.aspx" type="button" onclick="topFunction()" id="myBtn" runat="server" title="Add New">
 ...
 <script>
     ...
     function ChangeStatus(StrId, stsmode, cnclusrId) {
         if (cnclusrId == "0") {
             ezBSAlert({
                 type: "confirm",
                 messageText: "Do you want to change the status of this insurance provider?",
                 alertType: "info"
             }).done(function (e) {
                 if (e == true) {
                     ...
                     var Details = PageMethods.ChangeStatus(StrId, stsmode, usrId, function (response) {
                         ...
                         if (SucessDetails == "success") {
                             window.location = 'gen_Insurance_provider_List.aspx?InsUpd=StsCh';
                         }
                         else {
                             window.location = 'gen_Insurance_provider_List.aspx?InsUpd=Error';
                         }
                     });
                 }
             });
     ...
     function PrintClick() {
         ...
         var Type = document.getElementById("<%=HiddenFieldType.ClientID%>").value;
         var TypeText = document.getElementById("<%=HiddenFieldTypeText.ClientID%>").value;
         if (corptID != "" && corptID != null && orgID != "" && orgID != null) {
             $.ajax({
                 type: "POST",
                 async: false,
                 contentType: "application/json; charset=utf-8",
                 url: "gen_Insurance_provider_List.aspx/PrintList",
                 data: '{orgID: "' + orgID + '",corptID: "' + corptID + '",Status: "' + Status + '",CnclSts: "' + CnclSts + '",Type: "' + Type + '",TypeText: "' + TypeText + '"}',
                 dataType: "json",
                 success: function (data) {
                     if (data.d != "") {
     ...
     <script>
         ...
         $(document).ready(function () {
             ...
             document.getElementById("<%=HiddenFieldType.ClientID%>").value = document.getElementById("<%=ddlProviderType.ClientID%>").value;
             document.getElementById("<%=HiddenFieldTypeText.ClientID%>").value = $("#cphMain_ddlProviderType option:selected").text();
         ...
         function Load_dt() {
             ...
             var url = "gen_Insurance_provider_List.aspx/LoadStaticDatafordt";
             $.ajax({
                 type: 'POST',
                 dataType: 'json',
                 contentType: "application/json; charset=utf-8",
         ...
         function getdata(strPageNumber) {
             ...
             var url = "gen_Insurance_provider_List.aspx/GetData";
             var objData = {};
             ...
             objData.EnableRecall = document.getElementById("<%=hiddenRoleRecall.ClientID%>").value;
             objData.Type = document.getElementById("<%=HiddenFieldType.ClientID%>").value;
             $.ajax({
