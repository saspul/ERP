<%@ Page Language="C#" MasterPageFile="~/MasterPage/MasterPageCompzit.master" AutoEventWireup="true" CodeFile="gen_Product_CategoryList.aspx.cs" Inherits="Master_gen_Product_Category_gen_Product_CategoryList" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function SuccessCancelation() {
            $("#success-alert").html("Category cancelled successfully.");
            $("#success-alert").fadeTo(2000, 500).slideUp(500, function () {
            });
        }
        ...
        function OpenCancelView(StrId) {
            ezBSAlert({
                type: "confirm",
                messageText: "Do you want to cancel this product category?",
                alertType: "info"
            }).done(function (e) {
                if (e == true) {
        ...
        function ReOpenConfByID(StrId, Mode, reasonmust, cnclRsn) {
            ...
            $.ajax({
                ...
                url: "gen_Product_CategoryList.aspx/ReOpenConfByID",
                data: '{orgID: "' + orgID + '",corptID: "' + corptID + '",userID: "' + userID + '",MasterDbId: "' + MasterDbId + '",Mode: "' + Mode + '",reasonmust: "' + reasonmust + '",cnclRsn: "' + cnclRsn + '"}',
                dataType: "json",
                success: function (data) {
                    window.location.href = "gen_Product_CategoryList.aspx?InsUpd=" + data.d;
                }
            });
            return false;
        }
        function SuccessSts() {
            $("#success-alert").html("Product category status changed successfully.");
            $("#success-alert").fadeTo(2000, 500).slideUp(500, function () {
            });
            return false;
        }
        function AlreadySts() {
            $("#divWarning").html("Sorry, This product category status is already changed!");
            $("#divWarning").fadeTo(3000, 500).slideUp(500, function () {
            });
            return false;
        }
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphMain" runat="Server">
   ...
   <asp:HiddenField ID="HiddenFieldCancelReasMust" runat="server" Value="0"/>
    ...
    <asp:HiddenField ID="hiddenCommodityValue" runat="server" value="0"/>
...
<!-------working area_closed---->
 ...
 <a href="#" type="button" class="print_o" title="Print page" onclick="return PrintClick();">
  <i class="fa fa-print"></i>
    ...
    <a href="gen_Product_Category.aspx" type="button" onclick="topFunction()" id="myBtn" runat="server" title="Add New">
 ...
 <script>
     ...
     function ChangeStatus(StrId, stsmode, cnclusrId) {
         if (cnclusrId == "0") {
             ezBSAlert({
                 type: "confirm",
                 messageText: "Do you want to change the status of this product category?",
                 alertType: "info"
             }).done(function (e) {
                 if (e == true) {
                     ...
                     var Details = PageMethods.ChangeStatus(StrId, stsmode, usrId, corptID, function (response) {
                         ...
                         if (SucessDetails == "success") {
                             window.location = 'gen_Product_CategoryList.aspx?InsUpd=StsCh';
                         }
                         else if (SucessDetails == "AlCncl") {
                             window.location = 'gen_Product_CategoryList.aspx?InsUpd=AlCncl';
                         }
                         else if (SucessDetails == "AlSts") {
                             window.location = 'gen_Product_CategoryList.aspx?InsUpd=AlSts';
                         }
                         else {
                             window.location = 'gen_Product_CategoryList.aspx?InsUpd=Error';
                         }
                     });
                 }
             });
     ...
     function PrintClick() {
         ...
         var intAccountSpecific = document.getElementById("<%=hiddenAccountSpecific.ClientID%>").value;
         var hiddenCommodityValue = document.getElementById("<%=hiddenCommodityValue.ClientID%>").value;         
         if (corptID != "" && corptID != null && orgID != "" && orgID != null) {
             $.ajax({
                 type: "POST",
                 async: false,
                 contentType: "application/json; charset=utf-8",
                 url: "gen_Product_CategoryList.aspx/PrintList",
                 data: '{orgID: "' + orgID + '",corptID: "' + corptID + '",Status: "' + Status + '",CnclSts: "' + CnclSts + '",intAccountSpecific: "' + intAccountSpecific + '",hiddenCommodityValue: "' + hiddenCommodityValue + '"}',
                 dataType: "json",
                 success: function (data) {
                     if (data.d != "") {
     ...
     <script>
         ...
         function Load_dt() {
             ...
             var objData = {};
             objData.intAccountSpecific = document.getElementById("<%=hiddenAccountSpecific.ClientID%>").value;
             objData.hiddenCommodityValue = document.getElementById("<%=hiddenCommodityValue.ClientID%>").value; 
             var url = "gen_Product_CategoryList.aspx/LoadStaticDatafordt";
             $.ajax({
                 type: 'POST',
         ...
         function getdata(strPageNumber) {
             ...
             var url = "gen_Product_CategoryList.aspx/GetData";
             var objData = {};
