...
public partial class Master_gen_Product_Master_gen_Product_MasterList : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            intUsrRolMstrId = Convert.ToInt32(clsCommonLibrary.USR_ROLE_MSTR.Product_Master);
            ...
            if (dtChildRol.Rows.Count > 0)
            {
                ...
                {
                    objEntityProduct.Corp_Id = Convert.ToInt32(Session["CORPOFFICEID"].ToString());
    ...
    public void Division_Load()
    {
        ...
        {
            ddlDivisionSearch.Items.Insert(0, "--SELECT--");
        }     
    }
    ...
    public static string ConvertDataTableToHTML(DataTable dt, int intEnableModify, int intEnableCancel)
    {
        ...
        {
            string strId = dt.Rows[intRowBodyCount][7].ToString();
            int intIdLength = dt.Rows[intRowBodyCount][7].ToString().Length;
            ...
            int intCancTransaction = Convert.ToInt32(dt.Rows[intRowBodyCount]["COUNT_TRANSACTION"].ToString());
                   ...
                   strHtml += "<tr  >";
                   strHtml += "<td class=\"tr_l\"><span style=\"width: 84%;margin:auto;float:left;word-break:break-word;\">" + dt.Rows[intRowBodyCount][0].ToString()+"</span><span style=\"width:16%;margin:auto;float:left;\">";
                   if (dt.Rows[intRowBodyCount]["PRDT_SALEABLE_STS"].ToString() == "1")
                   {
                       strHtml += " <span class=\"pull-right blu_c\" title=\"Saleable\"><i class=\"\">";
                       strHtml += "<i class=\"fa fa-shopping-cart \"></i>";
                       strHtml += "</span> ";
                   }
                   if (dt.Rows[intRowBodyCount]["PRDT_STOCKABLE_STS"].ToString() == "1")
                   {
                       strHtml += "<span class=\"pull-right blu_s\" title=\"Stockable\">";
                       strHtml += "<i class=\"fa fa-archive \"></i>";
                       strHtml += "</span>";
                   }
                   strHtml += " </span></td>";
                   ...
                   {
                       if (dt.Rows[intRowBodyCount][6].ToString() == "ACTIVE")
                   ...
                   if (intEnableModify == Convert.ToInt32(clsCommonLibrary.StatusAll.Active))
                   {
                       if (intCnclUsrId == 0)
                       {
                           strHtml += " <button class=\"btn act_btn bn1 bt_e\"  title=\"Edit\" onclick='return getdetails(\"gen_Product_Master.aspx?Id=" + Id + "\");'>"
                              + "<i class=\"fa fa-edit\"></i>" + "</button>";
                       }
                       else
                       {
                           strHtml += " <button  class=\"btn act_btn bn4 bt_v\" title=\"View\" onclick='return getdetails(\"gen_Product_Master.aspx?ViewId=" + Id + "\");'>" +
    ...
    public static string ReOpenConfByID(string orgID, string corptID, string userID, string MasterDbId, string Mode, string reasonmust, string cnclRsn)
    {
        ...
        try
        {
            ...
            objEntityProduct.User_Id = Convert.ToInt32(userID);
            ...
            if (Mode == "0")
            {
                if (reasonmust == "1")
                {
                    objEntityProduct.Cancel_Reason = cnclRsn;
                }
                else
                {
                    objEntityProduct.Cancel_Reason = objCommon.CancelReason();
                }
                ...
                if (dtComplaintDetail.Rows.Count > 0)
                {
                    ...
                    {
                        objBusinessLayerProduct.CancelProductMaster(objEntityProduct);
                        sts = "Cncl";
                    }
                    else
                    {
    ...
    public static string ChangeStatus(string strmemotId, string strStatus, string UsrId, string corptID)
    {
        ...
        clsEntityProduct_Master objEntityProduct = new clsEntityProduct_Master();
        objEntityProduct.Product_Id = Convert.ToInt32(strId);
        objEntityProduct.User_Id = Convert.ToInt32(UsrId);
        objEntityProduct.D_Date = System.DateTime.Now;
        objEntityProduct.Corp_Id = Convert.ToInt32(corptID);
        if (strStatus == "1")
        {
        ...
        objEntityProduct.Status = Convert.ToInt32(strStatus);
        ...
        if (dtComplaintDetail.Rows.Count > 0)
        {
            if (dtComplaintDetail.Rows[0]["PRDT_CNCL_USR_ID"].ToString() == "" || dtComplaintDetail.Rows[0]["PRDT_CNCL_USR_ID"].ToString() == null)
            {
                if (dtComplaintDetail.Rows[0]["PRDT_STATUS"].ToString() != strStatus)
                {
                    objBusinessLayerProduct.UpdateProductSts(objEntityProduct);
                    strRet = "success";
                }
                else
                {
    ...
    [WebMethod]
    public static string PrintList(string orgID, string corptID, string Status, string CnclSts, string Divs, string divText)
    {
        string strReturn = "";
        //end
        ...
        if (Divs != "--SELECT--" && Divs != "")
        {
            objEntityProduct.DivsionId = Convert.ToInt32(Divs);
        }
        DataTable dtUser = new DataTable();
        dtUser = objBusinessLayerProduct.ReadProductListBySearch(objEntityProduct);

        string strRet = "";
        string strImagePath = objCommon.GetImagePath(clsCommonLibrary.IMAGE_SECTION.PRODUCT_MASTER_PDF);
        objEntityCommon.SectionId = Convert.ToInt32(clsCommonLibrary.Section.PRODUCT_MASTER_PDF);
        ...
        string strImageName = "ProductList_" + corptID + "_" + strNextNumber + ".pdf";
        ...
        try
        {
            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
            {
                ...
                //adding table to pdf
                    ...
                    TBCustomer.AddCell(new PdfPCell(new Phrase("PRODUCT NAME", FontFactory.GetFont("Calibri", 8, Font.BOLD, BaseColor.WHITE))) { HorizontalAlignment = Element.ALIGN_LEFT, Padding = 2, BorderColor = FontGray, BackgroundColor = FontColour });
                    ...
                    TBCustomer.AddCell(new PdfPCell(new Phrase("PRODUCT CATEGORY", FontFactory.GetFont("Calibri", 8, Font.BOLD, BaseColor.WHITE))) { HorizontalAlignment = Element.ALIGN_LEFT, Padding = 2, BorderColor = FontGray, BackgroundColor = FontColour });
    ...
    public class PDFHeader : PdfPageEventHelper
    {
        ...
        public override void OnStartPage(iTextSharp.text.pdf.PdfWriter writer, iTextSharp.text.Document document)
        {
            ...
            headtable.AddCell(new PdfPCell(new Phrase("PRODUCT LIST ", new Font(FontFactory.GetFont("Calibri", 9, Font.BOLD, BaseColor.BLACK)))) { Border = 0, HorizontalAlignment = Element.ALIGN_LEFT });
            if (strImageLogo != "")
            {
    ...
    [WebMethod]
    public static string[] GetData(string OrgId, string CorpId, string ddlStatus, string CancelStatus, string EnableModify, string EnableCancel, string PageNumber, string PageMaxSize, string strCommonSearchTerm, string OrderColumn, string OrderMethod, string strInputColumnSearch, string Divs)
    {
        clsBusinessLayer objBusinessLayer = new clsBusinessLayer();
        clsBusinessLayerProductMaster objBusinessLayerProduct = new clsBusinessLayerProductMaster();
        clsEntityProduct_Master objEntityProduct = new clsEntityProduct_Master();
        if (ddlStatus == "2")
            ...
            ddlStatus = "2";
        objEntityProduct.Status = Convert.ToInt32(ddlStatus);
        objEntityProduct.Cancel_Status = Convert.ToInt32(CancelStatus);
        objEntityProduct.Org_Id = Convert.ToInt32(OrgId);
        objEntityProduct.Corp_Id = Convert.ToInt32(CorpId);
        if (Divs != "--SELECT--" && Divs != "")
        {
            objEntityProduct.DivsionId = Convert.ToInt32(Divs);
        }
        string[] strResults = new string[2];
        objEntityProduct.PageNumber = Convert.ToInt32(PageNumber);
        objEntityProduct.PageMaxSize = Convert.ToInt32(PageMaxSize);
        objEntityProduct.OrderMethod = Convert.ToInt32(OrderMethod);
        objEntityProduct.OrderColumn = Convert.ToInt32(OrderColumn);
        objEntityProduct.CommonSearchTerm = strCommonSearchTerm;
        ...
        objEntityProduct.SearchName = strSearchInputs[Convert.ToInt32(SearchInputColumns.NAME)];
        objEntityProduct.SearchCode = strSearchInputs[Convert.ToInt32(SearchInputColumns.CODE)];

        DataTable dt = new DataTable();
        dt = objBusinessLayerProduct.ReadProductListBySearch(objEntityProduct);
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            //Pagination
            strResults[1] = objBusinessLayer.GenereatePagination(intTotalItems, objEntityProduct.PageNumber, objEntityProduct.PageMaxSize, intCurrentRowCount);
        }
    ...
    public static string[] LoadStaticDatafordt()//Filters
    {
        ...
        foreach (var item in values)
        {
            ...
            {
                sbSearchInputColumns.Append("<th id=\"tdColumnHead_1\" onclick=\"SetOrderByValue(1)\" class=\"sorting th_b11 tr_l\" style=\"word-wrap:break-word;\">PRODUCT NAME<i class=\"fa fa-sort pull-right hed_fa\" aria-hidden=\"true\"></i><br><input autocomplete=\"off\" id=\"txtSearchColumn_" + Convert.ToInt32(item) + "\"  onkeypress=\"return isTagEnter(event);\" onkeydown=\"return isTagEnter(event);\" onkeyup=\"return SettypingTimer(event);\" class=\"tb_inp_1 tb_in\" type=\"text\" title=\"PRODUCT NAME\" placeholder=\"Product Name\"></th>");
            }
            else if (Convert.ToInt32(item).ToString() == "1")
            {

                sbSearchInputColumns.Append("<th id=\"tdColumnHead_2\" onclick=\"SetOrderByValue(2)\" class=\"sorting th_b8 tr_l\" style=\"word-wrap:break-word;\">PRODUCT CODE<i class=\"fa fa-sort pull-right hed_fa\" aria-hidden=\"true\"></i><br><input autocomplete=\"off\" id=\"txtSearchColumn_" + Convert.ToInt32(item) + "\"  onkeypress=\"return isTagEnter(event);\" onkeydown=\"return isTagEnter(event);\" onkeyup=\"return SettypingTimer(event);\" class=\"tb_inp_1 tb_in\" type=\"text\" title=\"PRODUCT CODE\" placeholder=\"Product Code\"></th>");
            }
        }
        sbSearchInputColumns.Append("<th id=\"tdColumnHead_3\"  onclick=\"SetOrderByValue(3)\" class=\"sorting th_b4 tr_l\" style=\"word-wrap:break-word;\">PRODUCT BRAND<i class=\"fa fa-sort pull-right hed_fa\" aria-hidden=\"true\"></i></th>");
        sbSearchInputColumns.Append("<th id=\"tdColumnHead_4\"  onclick=\"SetOrderByValue(4)\" class=\"sorting th_b4 tr_l\" style=\"word-wrap:break-word;\">PRODUCT GROUP<i class=\"fa fa-sort pull-right hed_fa\" aria-hidden=\"true\"></i></th>");
        sbSearchInputColumns.Append("<th id=\"tdColumnHead_5\"  onclick=\"SetOrderByValue(5)\" class=\"sorting th_b4 tr_l\" style=\"word-wrap:break-word;\">PRODUCT CATEGORY<i class=\"fa fa-sort pull-right hed_fa\" aria-hidden=\"true\"></i></th>");
        sbSearchInputColumns.Append("<th id=\"tdColumnHead_6\"  onclick=\"SetOrderByValue(6)\" class=\"sorting th_b8 tr_l\" style=\"word-wrap:break-word;\">DIVISION<i class=\"fa fa-sort pull-right hed_fa\" aria-hidden=\"true\"></i></th>");
        sbSearchInputColumns.Append("<th class=\"th_b4\"  style=\"word-wrap:break-word;\">ACTIONS</th>");
