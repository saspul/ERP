...
{//Functional
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            hiddenPermitFileSize.Value = Convert.ToInt32(clsCommonLibrary.IMAGE_SIZE.CANDIDATE_PROFILEPIC).ToString();
        }

        if (HiddenCandidateId.Value.ToString() != "")
        {
    ...
    //    //updateDepent(objEntityDependent.EmpUserId.ToString(), "");
    ...
    [WebMethod]
    public static string SaveDependant(string strId, string User_Id, string strOrgIdID, string strCorpID, string txtDepntName, string ddlRelationship, string txtOccupation, string txtDob)
    {
        clsBusinessLayerStaffFamilyDtls objBusinessDependent = new clsBusinessLayerStaffFamilyDtls();
        clsEntityLayerFamilyDetails objEntityDependent = new clsEntityLayerFamilyDetails();
        clsCommonLibrary objCommon = new clsCommonLibrary();
        ...
        if (User_Id != "")
        {
            objEntityDependent.User_Id = Convert.ToInt32(User_Id);
        }
         if (strOrgIdID != "")
        {
            objEntityDependent.Organisation_id = Convert.ToInt32(strOrgIdID);
        }
         if (strCorpID != "")
        {
            objEntityDependent.Corporate_id = Convert.ToInt32(strCorpID);
        }
    

        objEntityDependent.EmpUserId = Convert.ToInt32(strId);
        objEntityDependent.Date = System.DateTime.Now;
        objEntityDependent.DepntName = txtDepntName;
        objEntityDependent.RelatnshpId = Convert.ToInt32(ddlRelationship);
        objEntityDependent.Occupation = txtOccupation;
        if (txtDob != "")
        {
            objEntityDependent.DateOfBirth = objCommon.textToDateTime(txtDob);
        }
    ...
    public static string ConvertDataTableToHTMLForDependent(DataTable dt)
    {
        ...
        for (int intRowBodyCount = 0; intRowBodyCount < dt.Rows.Count; intRowBodyCount++)
        {
            ...
            strHtml += "<td class=\"tdT\" style=\"width:4%; word-wrap:break-word;text-align: center;\">" + " <a class=\"tooltip\" title=\"Delete\" style=\"margin-top: -1.5%;opacity: 1;cursor: pointer;margin-left:1%;\" onclick=\"return deleteDepntById('" + strId + "');\" >" +
                                "<img  style=\"cursor: pointer;margin-top: -1.5%;opacity: 1;\" src='/Images/Icons/delete.png' /> " + "</a> </td>";
    ...
    public void updateDepent(string id, string mode)
    {
        ...
        //        ScriptManager.RegisterStartupScript(this, GetType(), "changeMaritalStatus", "changeMaritalStatus();", true);
    ...
    public static Dependent ReadDepntDtlById(string Id)
    {
        ...
        if (dt.Rows.Count > 0)
        {
          ...
          //  objDepnt.age = dt.Rows[0]["STAFF_DPNT_AGE"].ToString();
    ...
    [WebMethod]
    public static Dependent ReadDepntList(string empId, int CorpId, int Orgid)
    {
        Dependent objDepnt = new Dependent();
        clsBusinessLayerStaffFamilyDtls objBusinessDependent = new clsBusinessLayerStaffFamilyDtls();
        clsEntityLayerFamilyDetails objEntityDependent = new clsEntityLayerFamilyDetails();
    ...
    protected void btnAddCloseSCD_Click(object sender, EventArgs e)
    {
        ...
        objEntityContactDtls.StaffPermanentAdd = txtPermAddrsSCD.Text.Trim().ToUpper();

        objEntityContactDtls.EmergencyContact = txtEmrgCntcSCD.Text.Trim().ToUpper(); 
    ...
    protected void btnAddPersnl_Click(object sender, EventArgs e)
    {
        ...
        int intImageSection = Convert.ToInt32(CL_Compzit.clsCommonLibrary.IMAGE_SECTION.CANDIDATE_PROFILEPIC);
        string strImgPath = objCommon.GetImagePath(CL_Compzit.clsCommonLibrary.IMAGE_SECTION.CANDIDATE_PROFILEPIC);
        if (FileUploadImage.HasFile)
        {
            // GET FILE EXTENSION

            string strFileExt;
            objEntityPersonalDtls.FileNameAct = FileUploadImage.FileName;
            strFileExt = FileUploadImage.FileName.Substring(FileUploadImage.FileName.LastIndexOf('.') + 1).ToLower();

            string strImageName = intImageSection.ToString() + "_" + HiddenCandidateId.Value + "." + strFileExt;
            objEntityPersonalDtls.FileName = strImageName;
        ...
        if (FileUploadImage.HasFile)
        {
            FileUploadImage.SaveAs(Server.MapPath(strImgPath) + objEntityPersonalDtls.FileName);
        }


        string strRandom = objCommon.Random_Number();

        string strId = HiddenCandidateId.Value;
        int intIdLength = HiddenCandidateId.Value.Length;
        string stridLength = intIdLength.ToString("00");
        string Id = stridLength + strId + strRandom;
        ...
        if (clickedButton.ID == "btnAddpersonal")
        {
            ...
            Response.Redirect("gen_Candidate_Personal_Informn.aspx?Id=" + Id + "&InsUpd=Save");
        //    ScriptManager.RegisterStartupScript(this, GetType(), "SuccessConfirmationPD", "SuccessConfirmationPD();", true);
        }
        else if (clickedButton.ID == "btnAddClosepersonal")
        {
            Response.Redirect("gen_Candidate_Personal_Info_List.aspx?InsUpd=Save");
        }
    ...
    public void Update_PersonalDtls(string strP_Id)
    {
        ...
        if (dtMnpwrrMstr.Rows.Count > 0)
        {
            ...
            if (dtMnpwrrMstr.Rows[0]["IMAGE_FILENAME"] != DBNull.Value && dtMnpwrrMstr.Rows[0]["IMAGE_FILENAME"].ToString() != "")
            {
                hiddenImageFile.Value = dtMnpwrrMstr.Rows[0]["IMAGE_FILENAME"].ToString();
                hiddenImageFileAct.Value = dtMnpwrrMstr.Rows[0]["IMAGE_FLNM_ACT"].ToString();
                string strFileName = dtMnpwrrMstr.Rows[0]["IMAGE_FILENAME"].ToString();
                //    divImageEdit.Visible = true;
                clsCommonLibrary objCommon = new clsCommonLibrary();
                string strImagePath = objCommon.GetImagePath(CL_Compzit.clsCommonLibrary.IMAGE_SECTION.CANDIDATE_PROFILEPIC) + dtMnpwrrMstr.Rows[0]["IMAGE_FILENAME"].ToString();
    ...
    protected void btnUpdatePersonalDtls_Click(object sender, EventArgs e)
    {
        ...
        if (FileUploadImage.HasFile)
        {
            ...
            string strImageName = intImageSection.ToString() + "_" + HiddenCandidateId.Value + "." + strFileExt;
            objEntityPersonalDtls.FileName = strImageName;
            hiddenImageFile.Value = strImageName;
        }

        else
        {
            if (hiddenImageFile.Value == "")
            {
                objEntityPersonalDtls.FileName = "";
                objEntityPersonalDtls.FileNameAct = "";
            }
            else
            {
                objEntityPersonalDtls.FileName = hiddenImageFile.Value;
                objEntityPersonalDtls.FileNameAct = hiddenImageFileAct.Value;
            }
        }
        ...
        if (FileUploadImage.HasFile)
        {
            if (hiddenImageFileDeleted.Value != "")
            {
                string imageLocation = strImgPath + hiddenImageFileDeleted.Value;
        ...
        if (clickedButton.ID == "btnUpdatepersonal")
        {
           // ScriptManager.RegisterStartupScript(this, GetType(), "SuccessUpdationPD", "SuccessUpdationPD();", true);
            Response.Redirect("gen_Candidate_Personal_Informn.aspx?Id=" + Id + "&InsUpd=Upd");
        }
        else if (clickedButton.ID == "btnUpdateClosepersonal")
        {
            Response.Redirect("gen_Candidate_Personal_Info_List.aspx?InsUpd=Upd");
        }
    }
    ...
    [WebMethod]
    public static string SaveStaffImigrationDtls(string strId, string User_Id, string strOrgIdID, string strCorpID, string ddlVisaType, string txtVisaNumber, string txtVisaExpityDate, string txtPassportNum, string txtPassportExpiryDate)
    {
        int intuserid = 0;
        clsBusinessLayerStaffImmigration objBusinessImigration = new clsBusinessLayerStaffImmigration();
        clsEntityStaffImmigration objEntityImigrationDtls = new clsEntityStaffImmigration();
        ...
        if (strCorpID != "")
        {
            objEntityImigrationDtls.CorpId = Convert.ToInt32(strCorpID);
        }
        if (strOrgIdID != "")
        {
            objEntityImigrationDtls.OrgId = Convert.ToInt32(strOrgIdID);
        }
        if (User_Id != "")
        {
            intuserid = Convert.ToInt32(User_Id);
            objEntityImigrationDtls.Imig_user_id = Convert.ToInt32(User_Id);
        }
        ...
        if (ddlVisaType != "")
        {
            objEntityImigrationDtls.intVisaTypeID = Convert.ToInt32(ddlVisaType);
        }

        objEntityImigrationDtls.VisaNo = txtVisaNumber;

        if (txtVisaExpityDate == "")
        {
            objEntityImigrationDtls.VisaExpDate = new DateTime();
        }
        else
        {
            objEntityImigrationDtls.VisaExpDate = objCommon.textToDateTime(txtVisaExpityDate);
        }
        objEntityImigrationDtls.PassNo = txtPassportNum;

        if (txtPassportExpiryDate == "")
        {
            objEntityImigrationDtls.PassExpDate = new DateTime();
        }
        else
        {
            objEntityImigrationDtls.PassExpDate = objCommon.textToDateTime(txtPassportExpiryDate);
        }

        objEntityImigrationDtls.UsrInsdate = DateTime.Now;
        objEntityImigrationDtls.Imig_Emp_id = intuserid;
    ...
    public static string ConvertDataTableToHTMLForImig(DataTable dt)
    {
    ...
    [WebMethod]
    public static immigration ReadImigrationList(string candid, string CorpId, string Orgid)
    {
    ...
    public void updateOtherDtls(string id)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            //}
            hiddenOtherDetailsMode.Value = "UPD";
    ...
    //    objEntityWorkExperience.WrkAddress = txtAddressOfEmployerWrkExp.Text.Trim().ToUpper();
    ...
    //    {
    //        objEntityWorkExperience.StrSalary = txtSalaryWrkExp.Text.Trim();
    //    }
    ...
    [WebMethod]
    public static string SaveWorkExperience(string strId, string User_Id, string strOrgIdID, string strCorpID, string NameOfEmployerWrkExp, string GCCExpYearsWrkExp, string WorkExpYearsWrkExp,
        string DateOfJoiningWrkExp, string DateOfLeavingWrkExp, string DesignationWrkExp, string SalaryWrkExp, string AddressOfEmployerWrkExp)
    {
        ...
        objEntityWorkExperience.Corporate_id = Convert.ToInt32(strCorpID);
        objEntityWorkExperience.Organisation_id = Convert.ToInt32(strOrgIdID);
        objEntityWorkExperience.User_id = Convert.ToInt32(User_Id);
        objEntityWorkExperience.CandidateID = Convert.ToInt32(strId);


        objEntityWorkExperience.WrkEmpName = NameOfEmployerWrkExp.Trim().ToUpper();
        objEntityWorkExperience.WrkAddress = AddressOfEmployerWrkExp.Trim().ToUpper();
        if (WorkExpYearsWrkExp.Trim() != "")
        {
            objEntityWorkExperience.WrkExpYears = Convert.ToDecimal(WorkExpYearsWrkExp.Trim());
        }
        if (GCCExpYearsWrkExp.Trim() != "")
        {
            objEntityWorkExperience.WrkGCCExpYears = Convert.ToDecimal(GCCExpYearsWrkExp.Trim());
        }
        if (DateOfJoiningWrkExp.Trim() != "")
        {
            objEntityWorkExperience.LastWrkJoiningDate = objCommon.textToDateTime(DateOfJoiningWrkExp);
        }
        if (DateOfLeavingWrkExp.Trim() != "")
        {
            objEntityWorkExperience.LastWrkLeavingDate = objCommon.textToDateTime(DateOfLeavingWrkExp);
        }
        objEntityWorkExperience.Designation = DesignationWrkExp.Trim().ToUpper();
        if (SalaryWrkExp.Trim() != "")
        {
            objEntityWorkExperience.StrSalary = SalaryWrkExp.Trim();
        }
        objEntityWorkExperience.Date = System.DateTime.Now;
    ...
    protected void btnUpdateWrkExp_Click(object sender, EventArgs e)
    {
        ...
        {
            objEntityWorkExperience.WrkExpYears = -1;
        }
        if (txtGCCExpYearsWrkExp.Text.Trim() != "")
        {
            objEntityWorkExperience.WrkGCCExpYears = Convert.ToDecimal(txtGCCExpYearsWrkExp.Text.Trim());
        ...
        {
            objEntityWorkExperience.WrkGCCExpYears = -1;
        }
        if (txtDateOfJoiningWrkExp.Text.Trim() != "")
        {
            objEntityWorkExperience.LastWrkJoiningDate = objCommon.textToDateTime(txtDateOfJoiningWrkExp.Text);
    ...
    public static string ConvertDataTableToHTMLwrkExp(DataTable dt)
    {
    ...
    [WebMethod]
    public static WorkExperience ReadWrkExpList(string CandID, string OrgIDStaff, string CorpidIDStaff)
    {
        WorkExperience objWrkExp = new WorkExperience();
    ...
    public void EduLvlLoad()
    {
        ...
        //}
              //HiddenddlQualificationSelectVal.Value = "0";
    }
    ...
    [WebMethod]
    public static string AddEducation(string strId,string User_Id,string strOrgIdID,string strCorpID,string EduType,string EduQualification,string EduSpecialization,string EduInstite,
        string EduPercentage, string MonthEdu, string YearEdu, string EduDegree, string startDate)
    {
        clsBusinessLayerStaffEducation objBusinessEducation = new clsBusinessLayerStaffEducation();
        clsEntityLayerStaffEducation objEntityEducation = new clsEntityLayerStaffEducation();
        clsCommonLibrary objCommon = new clsCommonLibrary();

        objEntityEducation.Corporate_id = Convert.ToInt32(strCorpID);
        objEntityEducation.Organisation_id = Convert.ToInt32(strOrgIdID);
        objEntityEducation.User_id = Convert.ToInt32(User_Id);

        objEntityEducation.CandidateID = Convert.ToInt32(strId);
        objEntityEducation.CourseID = Convert.ToInt32(EduQualification);

        objEntityEducation.Institution = EduInstite;
        objEntityEducation.Specialization = EduSpecialization;
        objEntityEducation.Degree = EduDegree;
        ...
        if (YearEdu != "--YEAR--" && MonthEdu != "--MONTH--")
        {
            // objEntityEducation.PassingYear = objCommon.textToDateTime(txtEduStrtDate.Text);
            int intYear = Convert.ToInt32(YearEdu);
            int intMonth = Convert.ToInt32(MonthEdu);
            DateTime dateMonthAndYearEdu = new DateTime(intYear, intMonth, 1);
            objEntityEducation.PassingYear = dateMonthAndYearEdu;
        }
        else
        ...
        if (EduPercentage != "")
        {
            objEntityEducation.Percentage = Convert.ToDecimal(EduPercentage);
        }
        objBusinessEducation.insertEducation(objEntityEducation);
    ...
    protected void btnUpdateEdu_Click(object sender, EventArgs e)
    {
        ...
        if (HiddenddlQualificationSelectVal.Value != "0")
        {
            objEntityEducation.CourseID = Convert.ToInt32(HiddenddlQualificationSelectVal.Value);
        }
    ...
    //It build the Html table by using the datatable provided
    public static string ConvertDataTableToHTMLeductn(DataTable dt)
    {
        ...
        for (int intRowBodyCount = 0; intRowBodyCount < dt.Rows.Count; intRowBodyCount++)
        {
            ...
            strHtml += "<td class=\"tdT\" style=\"width:4%; word-wrap:break-word;text-align: center;\">" + "<a style=\"cursor:pointer;opacity: 1;margin-left: 1%;margin-top: -1%;z-index: 99;\" class=\"tooltip\"  title=\"Edit\" onclick=\"return updateEduDtlById('" + strId + "','" + strCandId + "');\" >" +
                            "<img  style=\"cursor: pointer;margin-left: -63%;\" src='/Images/Icons/edit.png' /> " + "</a> </td>";
    ...
    [WebMethod]
    public static Education ReadEduList(string empId)
    {
        Education objEdu = new Education();
        clsBusinessLayerStaffEducation objBusinessEducation = new clsBusinessLayerStaffEducation();
        clsEntityLayerStaffEducation objEntityEducation = new clsEntityLayerStaffEducation();
    ...
    [WebMethod]
    public static string SaveLangDtls(string strId, string User_Id, string strOrgIdID, string strCorpID, string LangId, string LangWrt, string LangRead, string LangSpk, string LangMotherTongue)
    {
        clsBusinessLayerStaffLanguage objBusinessLangauage = new clsBusinessLayerStaffLanguage();
        clsEntityLayerStaffLanguage objEntityLanguage = new clsEntityLayerStaffLanguage();
        clsCommonLibrary objCommon = new clsCommonLibrary();

        objEntityLanguage.Corporate_id = Convert.ToInt32(strCorpID);
        objEntityLanguage.Organisation_id = Convert.ToInt32(strOrgIdID);
        objEntityLanguage.User_id = Convert.ToInt32(User_Id);
        objEntityLanguage.CandidateID = Convert.ToInt32(strId);

        objEntityLanguage.LanguageId = Convert.ToInt32(LangId);

        objEntityLanguage.LangWrite = Convert.ToInt32(LangWrt);
        objEntityLanguage.LangRead = Convert.ToInt32(LangRead);
        objEntityLanguage.LangSpeak = Convert.ToInt32(LangSpk);
        objEntityLanguage.MotherTongue = Convert.ToInt32(LangMotherTongue);
    ...
    //It build the Html table by using the datatable provided
    public static string ConvertDataTableToHTMLlang(DataTable dt)
    {
    ...
    [WebMethod]
    public static Language ReadLangList(string CandID)
    {
        Language objLang = new Language();
        clsBusinessLayerStaffLanguage objBusinessLangauage = new clsBusinessLayerStaffLanguage();
        //clsQualification
    ...
    //    objEntityDependent.Maritalstst = Convert.ToInt32(rblMarrStatus.SelectedValue);
