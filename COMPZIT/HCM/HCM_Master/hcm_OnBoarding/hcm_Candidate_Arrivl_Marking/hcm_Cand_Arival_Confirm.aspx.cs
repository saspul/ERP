...
public partial class HCM_HCM_Master_hcm_OnBoarding_hcm_Candidate_Arrivl_Marking_hcm_Cand_Arival_Confirm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["Id"] != null)
        {
            ...
            lblEntry.Text = "Edit Candidate Arrival Marking";
    ...
    protected void btnCnfrm_Click(object sender, EventArgs e)
    {
       save();
        clsEntityLayer_Candidate_ShortList ObjEntityShortList = new clsEntityLayer_Candidate_ShortList();
        clsBusinessCandidate_ShortList objbusinessShortList = new clsBusinessCandidate_ShortList();
        clsBusiness_Arrivl_Confrmtn objbusinessArrivl_Cnfrm = new clsBusiness_Arrivl_Confrmtn();
        clsEntity_Arrivl_Confrmtn objEntityArrival=new clsEntity_Arrivl_Confrmtn();
        ...
        if (Session["CORPOFFICEID"] != null)
        {
            intCorpId = Convert.ToInt32(Session["CORPOFFICEID"].ToString());
            ObjEntityShortList.CorpOffice_Id = Convert.ToInt32(Session["CORPOFFICEID"].ToString());
            objEntityArrival.CorpOffice=Convert.ToInt32(Session["CORPOFFICEID"].ToString());
        }
        else if (Session["CORPOFFICEID"] == null)
        {
            Response.Redirect("~/Default.aspx");
        ...
        if (Session["ORGID"] != null)
        {
            intOrgId = Convert.ToInt32(Session["ORGID"].ToString());
            ObjEntityShortList.Organisation_Id = Convert.ToInt32(Session["ORGID"].ToString());
            objEntityArrival.Orgid= Convert.ToInt32(Session["ORGID"].ToString());
        }
        else if (Session["ORGID"] == null)
        {
            Response.Redirect("/Default.aspx");
        ...
        if (Session["USERID"] != null)
        {
            // intUserId = Convert.ToInt32(Session["USERID"]);
            ObjEntityShortList.User_Id = Convert.ToInt32(Session["USERID"]);
            objEntityArrival.UserId=Convert.ToInt32(Session["USERID"]);
        }
        else if (Session["USERID"] == null)
        {
            Response.Redirect("/Default.aspx");
        ...
        if (dtShortlist.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < dtShortlist.Rows.Count; i++)
            {
                if (dtShortlist.Rows[i]["CNFRM_STATS"].ToString() == "0")
                {
                    objEntityArrival.CandId = Convert.ToInt32(dtShortlist.Rows[i]["CAND_ID"].ToString());
                    objbusinessArrivl_Cnfrm.StatusChangeArrvlConfrmtn(objEntityArrival);
                    objbusinessArrivl_Cnfrm.Intw_skip_StatusChange(objEntityArrival);
                }
            }
