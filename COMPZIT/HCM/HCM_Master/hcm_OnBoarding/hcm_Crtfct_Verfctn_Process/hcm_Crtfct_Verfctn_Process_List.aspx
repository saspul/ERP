<%@ Page Language="C#" AutoEventWireup="true"  MasterPageFile="~/MasterPage/MasterPageCompzit_Hcm.master" CodeFile="hcm_Crtfct_Verfctn_Process_List.aspx.cs" Inherits="HCM_HCM_Master_hcm_OnBoarding_hcm_Crtfct_Verfctn_Process_hcm_Crtfct_Verfctn_Process_List" %>
  ...
  <style>
    ...
    <script src="/JavaScript/jquery-1.8.3.min.js"></script>
      <script type="text/javascript">
          ...
          $noCon(window).load(function () {
              ...
              document.getElementById("<%=CheckComplt.ClientID%>").focus();
              if (CancelPrimaryId != "") {
                  OpenCancelView();
              }
    ...
    <script type="text/javascript">
        function SuccessConfirmation() {
            ...
            document.getElementById("<%=lblMessageArea.ClientID%>").innerHTML = "Certificate validation process inserted successfully.";
        }
        function SuccessUpdation() {
            document.getElementById('divMessageArea').style.display = "";
            document.getElementById('imgMessageArea').src = "/Images/Icons/imgMsgAreaInfo.png";
            document.getElementById("<%=lblMessageArea.ClientID%>").innerHTML = "Certificate validation process updated successfully.";
        }
        function SuccessCancelation() {
            document.getElementById('divMessageArea').style.display = "";
            document.getElementById('imgMessageArea').src = "/Images/Icons/imgMsgAreaInfo.png";
            document.getElementById("<%=lblMessageArea.ClientID%>").innerHTML = "Certificate validation process cancelled successfully.";
        }
        function SuccessStatusChange() {
            document.getElementById('divMessageArea').style.display = "";
            document.getElementById('imgMessageArea').src = "/Images/Icons/imgMsgAreaInfo.png";
            document.getElementById("<%=lblMessageArea.ClientID%>").innerHTML = "Certificate validation process status changed successfully.";
        }
   ...
   <%--  for giving pagination to the html table--%>
    ...
    <script>
        ...
        function SearchValidation() {
            ...
            var cbxStatusComplt = document.getElementById("<%=CheckComplt.ClientID%>");
            var cbxComplt = 0;
            if (cbxStatusComplt.checked) {
                cbxComplt = 1;
            }
            else {
                cbxComplt = 0;
            ...
            if (ret == true) {
                ...
                document.getElementById("<%=HiddenSearchField.ClientID%>").value = cbx+','+cbxComplt;
            }
            ...
            if (confirm("Do you want to change the status of this entry?")) {
                ...
                var Details = PageMethods.ChangeContractStatus(CatId, CatStatus, function (response) {
                    ...
                    if (SucessDetails == "success") {
                        if (SearchString == "") {
                            window.location = 'gen_Interview_Temp_Master_List.aspx?InsUpd=StsCh';
                        }
                        else {
                            window.location = 'gen_Interview_Temp_Master_List.aspx?InsUpd=StsCh&Srch=' + SearchString + '';
                        }


                    }
                    else {
                        window.location = 'gen_Interview_Temp_Master_List.aspx?InsUpd=Error';
                    }
                });
            }
            else {
        ...
        function chkboxChangeCom()
        {
            if (document.getElementById("<%=CheckComplt.ClientID%>").checked == true) {
                document.getElementById("<%=cbxCnclStatus.ClientID%>").checked = false;
                document.getElementById("<%=cbxCnclStatus.ClientID%>").disabled = true;
            }
            else if (document.getElementById("<%=CheckComplt.ClientID%>").checked == false) {
               // document.getElementById("<%=cbxCnclStatus.ClientID%>").checked = false;
                document.getElementById("<%=cbxCnclStatus.ClientID%>").disabled = false;
            }
        }
        function chkboxChangedel() {
            if (document.getElementById("<%=cbxCnclStatus.ClientID%>").checked == true) {
                document.getElementById("<%=CheckComplt.ClientID%>").checked = false;
                document.getElementById("<%=CheckComplt.ClientID%>").disabled = true;
            }
            else if (document.getElementById("<%=cbxCnclStatus.ClientID%>").checked == false) {
                //document.getElementById("<%=CheckComplt.ClientID%>").checked = false;
                document.getElementById("<%=CheckComplt.ClientID%>").disabled = false;
            }
        }
        function CertValdId(id) {

            var nWindow = window.open('/HCM/HCM_Master/hcm_OnBoarding/hcm_Crtfct_Verfctn_Process/hcm_Crtfct_Verfctn_Process.aspx?ViewId=' + id + '&RFGP=VW', 'PoP_Up', 'width=1200,height=500,left=70,top=100,directories=no,navigationbar=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,resizable=yes,scrollbars=yes');
            // nWindow.focus();
        }
   ...
   <div id="divMessageArea" style="display: none">
    ...
    <div class="cont_rght">
        ...
        <div id="divReportCaption" style="font-size: 24px; font-weight: bold; color: rgb(83, 101, 51); font-family: Calibri;">
            <img src="/Images/BigIcons/Certificate_Validation-bg.PNG" style="vertical-align: middle;" />
   Certificate Validation 
        </div >
        <div style="width: 93.5%;margin-top:0.5%;height:10%;">

            <div style="width:100%;float:left;margin-top:14px">
                       
                        <div class="subform" style="width:19%;float: left;margin-left: 43%;margin-top: 0%;">


                    <asp:CheckBox ID="CheckComplt" Text="" runat="server" onkeydown="return DisableEnter(event);"  onclick="chkboxChangeCom()" Checked="false" class="form2" />
                    <h3 style="margin-top:1%;">Show Completed Entries</h3>
                  </div>

                                 
                <div class="subform" style="width:19%;float: left;margin-left: 4%;margin-top: 0%;">


                    <asp:CheckBox ID="cbxCnclStatus" Text="" runat="server" onkeydown="return DisableEnter(event);" onclick="chkboxChangedel()" Checked="false" class="form2" />
             ...
             <div style="width:12%;float:right;margin-top:-2.5%;margin-left: 0%;">
        ...
        <div onclick="location.href='hcm_Crtfct_Verfctn_Process.aspx'" id="divAdd" class="add" runat="server" style=" position: fixed; height:26.5px; right:1%;">
            ...
            <div id="MymodalCancelView" class="modalCancelView" >
                ...
                <div class="modal-CancelView">
                    <div class="modal-headerCancelView">
                        ...
                        <h3 style="font-family: Calibri; font-size: 18px; margin-left: 37%; padding-bottom: 0.7%; padding-top: 0.6%;"> Certificate Validation</h3>
                    </div>
