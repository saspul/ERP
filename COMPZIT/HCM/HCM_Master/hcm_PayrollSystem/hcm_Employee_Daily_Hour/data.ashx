        ...
        public static System.Collections.Generic.IEnumerable<Person> GetPersons(string Corpt_Id, string Org_Id, string Month, string Year)
        {
          ...
          //int cout = 0;
            foreach (DataRow dtRowsIn in dtorglist.Rows)
            {
                ...
                objEntityEmpDailyWorkHour.StrDate = DateMonth + "-" + DateYear;
                objEntityEmpDailyWorkHour.UserId = 1;
                objEntityEmpDailyWorkHour.AttandanceMode = 1;
               // DataTable dtConfCountList = objBusinessEmpDailyWorkHour.checkAllConfReop(objEntityEmpDailyWorkHour);
               // string strView = "<span class=\"responsiveExpander\"></span><button runat=\"server\" class=\"btn btn-xs btn-default\" title=\"View\" onclick=\"return ViewRow('" + DateMonth + "','" + DateYear + "');\"><i class=\"fa fa-eye\"></i></button>";
                string strView = "<span class=\"responsiveExpander\"></span><button runat=\"server\" class=\"btn btn-xs btn-default\" title=\"View\" onclick=\"return ViewRow('" + DateMonth + "','" + DateYear + "','" + dtRowsIn["TABLE_STS"].ToString() + "');\"><i class=\"fa fa-eye\"></i></button><button runat=\"server\" class=\"btn btn-xs btn-default\" title=\"Confirm\" onclick=\"return ConfirmAll('" + DateMonth + "','" + DateYear + "',0,'" + dtRowsIn["TABLE_STS"].ToString() + "');\" style=\"margin-left: 4%;\"><i class=\"fa fa-check\"></i></button>";
                if (dtRowsIn[3].ToString() == "0")
                {
                    strView = "<span class=\"responsiveExpander\"></span><button runat=\"server\" class=\"btn btn-xs btn-default\" title=\"View\" onclick=\"return ViewRow('" + DateMonth + "','" + DateYear + "','" + dtRowsIn["TABLE_STS"].ToString() + "');\"><i class=\"fa fa-eye\"></i></button><button runat=\"server\" class=\"btn btn-xs btn-default\" title=\"Confirm\" onclick=\"return ConfNotPossible();\" style=\"opacity: 0.4;margin-left: 4%;\"><i class=\"fa fa-check\"></i></button>";
                }
                 if (dtRowsIn[4].ToString() == "0")
                //if (dtConfCountList.Rows[0][0].ToString() == "0")
                {
                    strView += "<button runat=\"server\" class=\"btn btn-xs btn-default\" title=\"Reopen\"  onclick=\"return ReOpenNotPossible();\" style=\"opacity: 0.4;margin-left: 4%;\"><i class=\"fa fa-unlock\"></i></button>";
                }
                else
                {
                    strView += "<button runat=\"server\" class=\"btn btn-xs btn-default\" title=\"Reopen\"  onclick=\"return ConfirmAll('" + DateMonth + "','" + DateYear + "',1,'" + dtRowsIn["TABLE_STS"].ToString() + "');\" style=\"margin-left: 4%;\"><i class=\"fa fa-unlock\"></i></button>";
                }
