...
public partial class AWMS_AWMS_Transaction_Leave_Allocation_Master_Leave_Allocation_Master : System.Web.UI.Page
{
    ...
    public static string LevTypOverRideDate(string LevTypId, string EmpId)
    {
        ...
        if (EmpId != "" && LevTypId != "" && EmpId != "--SELECT AN EMPLOYEE--" && LevTypId != "--SELECT LEAVE TYPE--")
        {
            objEntityLeaveType.EmployeeId = Convert.ToInt32(EmpId);
            objEntityLeaveType.LeaveTypeMasterId = Convert.ToInt32(LevTypId);
    ...
    protected void btnConfirm_Click(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["Id"] != null)
        {
            ...
            objEntityLeaveRequest.Leave_Id = objEntLevAllocn.Leave_Id;
            DataTable dtD = objBusinessLeaveRequest.CheckTrvlDtlShow(objEntityLeaveRequest);
            if (dtD.Rows.Count > 0)
            {
                if (dtD.Rows[0]["LEAVETYP_EXC_SAL_PROC"].ToString() == "1")
                {
                    intLeaveDeductSts = 0;
                }
            }
       ...
       hiddenCurrentDate.Value = "01-01-2019";
        ...
        if (dtLeavDetail.Rows.Count > 0)
        {
            ...
            if (cbxStatus.Checked == true)
            {
                ...
                if (ddlSecTo.Items.FindByValue(dtLeavDetail.Rows[0]["LEAVE_TO_SCTN"].ToString()) != null)
                {
    ...
    //Start:-Code0009
    [WebMethod]
    public static string[] ReadDutyofChkDateRanges(string strdateFm, string orgid, string corpid, string strdateTo, string HoliPaidSts, string OffPaidSts)
    {
    ...
    public static string[] CheckTrvlDtlShow(int LeaveTypeId)
    {
        ...
        //Start:-Code0009
        strJson[4] = dt.Rows[0]["LEAVETYP_HOLIDAY_PAID_STS"].ToString();
        strJson[5] = dt.Rows[0]["LEAVETYP_OFFDAY_PAID_STS"].ToString();
        //End:-Code0009
        return strJson;
