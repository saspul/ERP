...
public partial class FMS_FMS_Master_fms_Debit_Note_fms_Debit_Note : System.Web.UI.Page
{
                    ...
                    if (dtLDGRdTLS.Rows[intCount]["LDGR_ID"].ToString() == dtLDGRdTLS.Rows[intCountinn]["LDGR_ID"].ToString())
                    {
                        if (first == 1)
                        {
                            ...
                            if (dtLDGRdTLS.Rows[intCountinn]["PURCHS_ID"].ToString() != "" && dtLDGRdTLS.Rows[intCountinn]["CST_CNTR_DR_AMOUNT"].ToString() != "")
                            {
                                ...
                                {
                                  //  drDtl["PURCHS_ID"] = dtLDGRdTLS.Rows[intCountinn]["PURCHS_ID"].ToString() + "%" + drDtl["CST_CNTR_CR_AMOUNT"];
                                    drDtl["PURCHS_ID"] = dtLDGRdTLS.Rows[intCountinn]["PURCHS_ID"].ToString() + "%" + drDtl["CST_CNTR_CR_AMOUNT"] + "%" + dtLDGRdTLS.Rows[intCountinn]["PURCHS_BAL_AMT"].ToString();
                        ...
                        else
                        {
                            ...
                            if (dtLDGRdTLS.Rows[intCountinn]["PURCHS_ID"].ToString() != "")
                            {
                                drDtl["PURCHS_ID"] = drDtl["PURCHS_ID"] + "$" + dtLDGRdTLS.Rows[intCountinn]["PURCHS_ID"].ToString() + "%" + drDtl["CST_CNTR_CR_AMOUNT"] + "%" + dtLDGRdTLS.Rows[intCountinn]["PURCHS_BAL_AMT"].ToString();
                            }
                        }
                    }
                }
    ...
    protected void bttnsave_Click(object sender, EventArgs e)
    {
        ...
        if (HiddenSaleDtls.Value != "" && HiddenSaleDtls.Value != null)
        {
            ...
            {
                if (objclsTVData.PURCHASEAMNT != "" && objclsTVData.PURCHASEID != "")
                {
                    ...
                    if (HiddenSaleDtls.Value != "" && HiddenSaleDtls.Value != null)
                    {
                        foreach (clsSaleData objclsTVData in objSaleDataList)
                        {
                            if (objclsTVData.PURCHASEAMNT != "" && objclsTVData.PURCHASEID != "")
                            {
                                ...
                                {
                                    DataTable dtSalesBalance = objBussinessCredit.ReadPurchaseBalance(objEntitySaleDtl);
