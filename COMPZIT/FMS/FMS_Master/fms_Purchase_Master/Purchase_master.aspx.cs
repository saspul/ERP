...
public partial class FMS_FMS_Master_Purchase_Matser_Purchase_master : System.Web.UI.Page
{
     ...
     //   BankCachLedgerLoad();
        ...
        if (dtPurchase.Rows.Count > 0)
        {
            ...
            else
            {
                ...
                //evm 0044
                if (dtPurchase.Rows[0]["PURCHS_CR_PRD"].ToString() != "")
                {
                    txtcrdtPeriod.Text = dtPurchase.Rows[0]["PURCHS_CR_PRD"].ToString();
                }
                //----------
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        if (cbxExtngSplr.Checked == true)
        {
            ...
            if (Hiddencustldgr.Value != "--SELECT SUPPLIER --" && Hiddencustldgr.Value != "")
            {
                objEntityPurchase.LedgerCustomer = Convert.ToInt32(Hiddencustldgr.Value);
            }
        }
        else
        {
        ...
        //evm 0044
        if (txtcrdtPeriod.Text != "")
        {
            objEntityPurchase.CreditPeriod = Convert.ToInt32(txtcrdtPeriod.Text.Trim());
        }
        else
        {
            objEntityPurchase.CreditPeriod = 0;
        }
        //------------
    ...
    protected void btnSupplier_Click(object sender, EventArgs e)
    {
        ...
        if (HiddenSupplierId.Value != "")
        {
            ...
            clsEntityPurchaseMaster objEntityPurchase = new clsEntityPurchaseMaster();
            //-----------
            objEntityPurchase.LedgerId = Convert.ToInt32(ddlCustomerLdgr.SelectedItem.Value);
            DataTable dt = objBusinesspurchase.ReadSupplierCreditsById(objEntityPurchase);
    ...
    //EVM 0044
    [WebMethod]
    public static string DisplayCreditDtls(string SupplierId)
    {

        clsBusiness_purchaseMaster objBusinesspurchase = new clsBusiness_purchaseMaster();
        clsEntityPurchaseMaster objEntityPurchase = new clsEntityPurchaseMaster();
        //-----------
        objEntityPurchase.LedgerId = Convert.ToInt32(SupplierId);
        DataTable dt = objBusinesspurchase.ReadSupplierCreditsById(objEntityPurchase);
