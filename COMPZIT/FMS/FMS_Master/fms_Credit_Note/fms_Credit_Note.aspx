<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage/MasterPageCompzit.master" AutoEventWireup="true" CodeFile="fms_Credit_Note.aspx.cs" Inherits="FMS_FMS_Master_fms_Credit_Note_fms_Credit_Note" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="cphHead" Runat="Server">
    ...
    <script>
                ...
                for (var key in json) {
                    if (json.hasOwnProperty(key)) {
                        if (json[key].CREDIT_NOTE_ID != "") {
                            EditListRows(json[key].CREDIT_NOTE_ID, json[key].LDGR_CR_ID, json[key].LDGR_ID, json[key].LDGR_CR_AMT, json[key].LDGR_NAME, json[key].CST_CNTR_CR_ID, json[key].COSTCNTR_ID, json[key].CST_CNTR_CR_AMOUNT, json[key].LDGR_CR_DR_CR_STATUS, json[key].SALES_ID, json[key].LDGR_REMARKS);
                        }
                    }
                }
                var x = 0;
        ...
        function EditListRows(CREDIT_NOTE_ID, LDGR_CR_ID, LDGR_ID, LDGR_CR_AMT, LDGR_NAME, CST_CNTR_CR_ID, COSTCNTR_ID, CST_CNTR_CR_AMOUNT, LDGR_CR_DR_CR_STATUS, SALES_ID, LDGR_REMARKS) {
            if (LDGR_ID != 0) {
            ...
            if (SALES_ID != null && SALES_ID != "" && SALES_ID != 0) {
                document.getElementById("tdSaleDtls" + currntx).value = SALES_ID;
            }
        ...
        function ddlLedOnchange(x, CedtOrDbt, mode) {
            ...
            else if (document.getElementById("TxtAmount_" + x).value != "") {
            ...
            if (Purchase_ret == true) {
                if (LedgerDuplication(x) == true) {
                    ...
                    if (document.getElementById("ddlRecptLedger" + x).value != "0") {
                        ...
                        var CreditID = "";
                        if (document.getElementById("<%=HiddenCreditId.ClientID%>").value != "")
                            CreditID = document.getElementById("<%=HiddenCreditId.ClientID%>").value;
                        var View = document.getElementById("<%=HiddenView.ClientID%>").value;
                        document.getElementById("ddlLedId" + x).value = LedgerId;
                        $noCon.ajax({
                            type: "POST",
                            url: "fms_Credit_Note.aspx/LoadSalesForLedger",
                            data: '{intLedgerId:"' + LedgerId + '",intorgid:"' + orgid + '" ,intcorpid:"' + corpid + '" ,strCedtOrDbt:"' + CedtOrDbt + '",strCurrencyId:"' + CurrencyId + '",x:"' + x + '",strCrncyAbrv:"' + CrncyAbrv + '" ,CreditID:"' + CreditID + '" ,View:"' + View + '" }',
                            ...
                            success: function (response) {
                                ...
                                if (mode != "Upd") {
                                    if (response.d[0] != "") {
                                        ...
                                        //document.getElementById("ModelHeading").innerHTML = "Sale";
                                            ...
                                            for (var Cst = 0; Cst < splitrow.length; Cst++) {
                                                ...
                                                for (var i = 1; i < addRowtable.rows.length; i++) {
                                                    ...
                                                    if (document.getElementById("tdSaleID" + P_Id).innerHTML == splitEach[0]) {
                                                        ...
                                                        document.getElementById("txtPurchaseRefSettleAmt" + P_Id).value = splitEach[3];
                                                    }
                                                }
                                            }
                                            settle = 1;//evm 0044 
                                            loadSettleAmount(P_Id);//evm 0044
                                        }
                                    ...
                                    var P_Id = 0;
                                    //  var row = $noCon('#TableAddQstn').find(' tbody tr:first').attr('id');
                                    var FLGSLS = 0;
                                    for (var i = 1; i < addRowtable.rows.length; i++) {
                                        if (FLGSLS == 0) {
                                    ...
                                    idlast = P_Id;
                                    setTimeout(function () { focusSale(idlast); }, 600);
                                }
                                if (mode == "Upd") {
                                    if (response.d[0] != "") {
                                        if (CedtOrDbt == "CDT") {
                                            ...
                                            document.getElementById("iSaleTag" + x).className = "fa fa-balance-scale ad_fa psc_s gre";
                                            document.getElementById("ChkSale" + x).disabled = false;
                                        }
                                        else {
                                            document.getElementById("ChkSale" + x).style.opacity = "0.3";
                                            //  document.getElementById("ChkSale" + x).style.backgroundColor = "#337ab7";
                                            document.getElementById("ChkSale" + x).disabled = true;
                                        }
                                    }
                                    else {
                                        document.getElementById("ChkSale" + x).style.opacity = "0.3";
            ...
            if (settle == "0") {
                setTimeout(function () { loadSettleAmount(x) }, 350);
                //evm 0044
            }
        ...
        function SuccessReoen() {
            var ret = false;
            $noCon("#success-alert").html("Credit note reopened successfully.");
        ...
        function Alreadyreopened() {
            var ret = false;
            $noCon("#divWarning").html("Credit note already reopened.");
        ...
        function SalesAmountExceeded() {
            $noCon("#divWarning").html(" Credit amount should not be greater than sales amount.");
        ...
        function PrintVersionWarning() {
            $noCon("#divWarning").html("Please define print version for credit note.");
            $noCon("#divWarning").fadeTo(3000, 500).slideUp(500, function () {
            });
            return false;
        }
        function OpenPrint() {
            ...
            var Id = document.getElementById("<%=HiddenCreditId.ClientID%>").value;
            ...
            if (corptID != "" && corptID != null && orgID != "" && orgID != null && UsrName != null && UsrName != "" && Id != "") {
                $.ajax({
                    ...
                    async: false,
                    contentType: "application/json; charset=utf-8",
                    url: "fms_Credit_Note.aspx/PrintPDF",
    ...
    <script>
        function MyModalCostCenter(x, y, CstCntr) {
            var SbCostCenter = '';
            SbCostCenter = '<div class=\"modal fade\" id=\"myModalCstCntr\" tabindex="-1"  data-backdrop=\"static\"  role=\"dialog\" aria-labelledby="exampleModalLabel" aria-hidden="true">';
            ...
            SbCostCenter += '<span class="spn_mod"  id=\"ModelHeading\"></span></h2>';
            ...
            SbCostCenter += '<div class=\"col-md-12\" >';

            SbCostCenter += '<div class=\"col-md-12 res_table_box\" >';
            SbCostCenter += '<div class=\"tab-content\" id=\"myTabContent\" >';
            SbCostCenter += '<div class=\"tab-pane fade active in\" id=\"home\" role=\"tabpanel\" aria-labelledby=\"home-tab\" >';

            SbCostCenter += '<ul class=\"list-group bg-grey\" >';
            ...
            //SbCostCenter += '<label for=\"example-text-input\" class=\"col-md-1 col-form-label\"><span></span></label>';
        ...
        function AddNewGroup(ledgerid) {
            ...
            FrecRow += '<div></div><div style="display:none" id="groupSubCat' + RowIndex1 + '">' + rowSubCatagory + '</div> ';
            ...
            FrecRow += '<input class="form-control" style="display:none" name="ddlLedId' + RowIndex1 + '"  value="0" id="ddlLedId' + RowIndex1 + '" type="text">';
            FrecRow += '</div><span id="AccntBalance_' + RowIndex1 + '" class="input-group-addon cur2"><i class="fa fa-money"></i></span></td>';

            FrecRow += '<td class="tr_r">';
            FrecRow += '<div class="input-group"> <span class="input-group-addon cur1"><label for="example-text-input" class="col-form-label" >' + document.getElementById("<%=HiddenCurrencyAbrv.ClientID%>").value + '</label></span>';
            ...
            FrecRow += '<button title="ADD"  id="CreditNote' + RowIndex1 + '" onclick="return FuctionAddGroup(\'' + RowIndex1 + '\')" class="btn act_btn bn2" ><i   class="fa fa-plus-circle"  style="display: block;">&nbsp;</i></button>';
            FrecRow += '<button title="DELETE" id="bttnRemovGrp' + RowIndex1 + '"   onclick="return removeRowGrps(' + RowIndex1 + ',\'Are you sure you want to delete this ledger?\')" class="btn act_btn bn3" ><i class="fa fa-trash"   style="display: block;">&nbsp;</i></button>';
            FrecRow += '</div></td>';


            FrecRow += '<td ><a title="SALES RETURN" id="ChkSale' + RowIndex1 + '" type="button" href="javascript:;" onclick="return ddlLedOnchange(\'' + RowIndex1 + '\',\'ins\',\'ins\');" ><i id="iSaleTag' + RowIndex1 + '" class="fa fa-balance-scale ad_fa psc_s"></i></a>';
            FrecRow += '<a title="COST CENTRE" id="ChkCostCenter' + RowIndex1 + '" type="button" href="javascript:;" onclick="MyModalCostCenter(\'' + RowIndex1 + '\',\'' + rowSubCatagory + '\',null);"> <i class="fa fa-filter ad_fa psc_c"></i></button></td>';
    ...
    if (document.getElementById("ddlRecptLedger" + x).value == "" || document.getElementById("ddlRecptLedger" + x).value == 0) {
        ...
        function PendingPurchase(textboxid, x, CedtOrDbt) {
            ...
            if (document.getElementById(textboxid).value != "") {
                ...
                document.getElementById(textboxid).value = TxtCstctrAmount;
          ...
          //      document.getElementById("TxtAmount_" + xtemp).style.borderColor = "";
              //  document.getElementById("TxtAmountCrdt" + xtemp).style.borderColor = "";
                ...
                if (LdAmt <= 0) {
                    ...
                    document.getElementById("TxtAmountCrdt" + xtemp).focus();
                        ...
                        $noCon.ajax({
                            ...
                            success: function (response) {
                                ...
                                if (response.d[1] != "") {
                                    ...
                                    document.getElementById("AccntBalance_" + varRowidx).innerHTML = "";
                                }

                                if (response.d[5] != "") {
                                    if (response.d[5] == "0" || response.d[5] == "1") {
                                        document.getElementById('ChkCostCenter' + varRowidx).style.pointerEvents = 'none';
                                        document.getElementById('ChkCostCenter' + varRowidx).style.opacity = "0.5";
                                    }
                                    else {
                                        document.getElementById('ChkCostCenter' + varRowidx).style.pointerEvents = 'auto';
                                        document.getElementById('ChkCostCenter' + varRowidx).style.opacity = "1";
                                    }
                                }
            ...
            for (var i = 0; i < addRowtable.rows.length; i++) {
                ...
                if (PurchaseInfo != "" && PurchaseInfo != null && PurchaseInfo != "null") {
                    ...
                    $.ajax({
                        ...
                        async: false,
                        contentType: "application/json; charset=utf-8",
                        url: "fms_Credit_Note.aspx/CheckSaleSettlement",
                        ...
                        for (var Cst = 0; Cst < splitrow.length; Cst++) {
                            ...
                            if (splitEach[0] != "") {
                                ...
                                var client = JSON.stringify({
                                    ...
                                    MAINTABID: "" + EvtGrp + "",
                                    SALEID: "" + splitEach[0] + "",
                                    SALEAMNT: "" + splitEach[1] + "",
                                    SALEAMNT_ACT: "" + splitEach[2] + "",
                                    SALES_SETTLEAMNT: "" + splitEach[3] + "",
                                    SALES_SETTLE_BALAMNT: "" + splitEach[4] + "",
                                });
        ...
        function ConfirmMessage() {
            //if (confirmbox > 0) {
                ezBSAlert({
                    ...
                    if (e == true) {
                        window.location.href = "fms_Credit_Note_List.aspx";
                    }
                    else {
                        return false;
                    }
        ...
        function SuccessUpdation() {
            $noCon("#success-alert").html("Credit note updated successfully.");
        ...
        function SuccessNotConfirmation() {
            $noCon("#divWarning").html("Credit note already confirmed.");
            $noCon("#divWarning").fadeTo(2000, 500).slideUp(500, function () {
            });
        ...
        function SuccessInsertion() {
            $noCon("#success-alert").html("Credit note inserted successfully.");
            $noCon("#success-alert").fadeTo(2000, 500).slideUp(500, function () {
            });
...
<asp:Content ID="Content2" ContentPlaceHolderID="cphMain" Runat="Server">
    ...
    <asp:HiddenField ID="HiddenCreditId" runat="server" />
    ...
    <asp:HiddenField ID="HiddenAuditCloseProvisionStatus" runat="server" />
    ...
    <asp:HiddenField ID="HiddenSequenceNum" runat="server" />
  ...
  <div id="divLinkSection" runat="server">
    <ol class="breadcrumb sticky1">
        ...
        <li><a href="fms_Credit_Note_List.aspx">Credit Note</a></li>
        ...
        <div class="content_area_container cont_contr">
            <div class="content_box1 cont_contr">
                <div class="" onmouseover="closesave()">
                    ...
                    <div class="fg2">
                        <label for="email" class="fg2_la1">Credit note REF #<span class="spn1">&nbsp;</span></label>
                    ...
                    <script>
                        ...
                        function showFromDate() {
                            ...
                            var CreditID = $('#cphMain_HiddenCreditId').val().trim();
                            var AcntPrvsn = document.getElementById("<%=HiddenFieldAcntCloseReopenSts.ClientID%>").value
                            var AuditPrvsn = document.getElementById("<%=HiddenAuditCloseProvisionStatus.ClientID%>").value
                            ...
                            if (jrnlDate != "") {
                                ...
                                $.ajax({
                                    ...
                                    async: false,
                                    contentType: "application/json; charset=utf-8",
                                    url: "fms_Credit_Note.aspx/CheckAcntCloseSts",
                            ...
                            if (jrnlDate != "" && (document.getElementById("<%=HiddenFieldAcntCloseReopenSts.ClientID%>").value == "1" || document.getElementById("<%=HiddenAuditCloseProvisionStatus.ClientID%>").value == "1")) {
                                // if (RcptDate != "") {
                                if (document.getElementById("<%=HiddenRefAccountCls.ClientID%>").value == "1") {
                                    ...
                                    $.ajax({
                                        ...
                                        async: false,
                                        contentType: "application/json; charset=utf-8",
                                        url: "fms_Credit_Note.aspx/CheckRefNumber",
                                        data: '{jrnlDate: "' + jrnlDate + '",orgID: "' + orgID + '",corptID: "' + corptID + '",usrID: "' + usrID + '",RefNum: "' + RefNum + '",CreditID: "' + CreditID + '"}',
                                        dataType: "json",
                                        success: function (data) {
                                            if (data.d != "") {
                        ...
                        function ButtnFillClickSales() {
                            ...
                            for (var i = 1; i < addRowtable.rows.length; i++) {
                                ...
                                var purchaseRefSettleAmt = 0;
                                if (document.getElementById("txtPurchaseRefSettleAmt" + P_Id).value != "") {
                                    purchaseRefSettleAmt = document.getElementById("txtPurchaseRefSettleAmt" + P_Id).value;
                                    purchaseRefSettleAmt = purchaseRefSettleAmt.replace(/\,/g, '');
                                }
                                ...
                                if (document.getElementById("tdSettld" + P_Id).value == "0") {//EVM-0020
                                    ...
                                    TotalPurchaseAmnt = parseFloat(TotalPurchaseAmnt) + parseFloat(purchaseRefSettleAmt);

                                    var TotalSaleAmnt = parseFloat(purchaseAmt) + parseFloat(purchaseRefSettleAmt);

                                    var tdSalesBalance = document.getElementById("tdSalesBalance" + P_Id).value;
                                    tdSalesBalance = tdSalesBalance.replace(/\,/g, '');
                                    var tdCreditNtBalance = document.getElementById("tdCreditBalance" + P_Id).value;
                                    tdCreditNtBalance = tdCreditNtBalance.replace(/\,/g, '');

                                    if (parseFloat(purchaseRefSettleAmt) > parseFloat(tdSalesBalance)) {
                                        document.getElementById("lblErrMsgCancelReason").innerHTML = "Entered amount should be less than the sales balance amount";
                                        document.getElementById("lblErrMsgCancelReason").style.display = "";
                                        document.getElementById("txtPurchaseRefSettleAmt" + P_Id).style.borderColor = "red";
                                        $("div.war").fadeIn(200).delay(500).fadeOut(400);
                                        ret = false;
                                    }

                                    if (purchaseAmt != "") {
                                        if (parseFloat(purchaseAmt) > parseFloat(tdCreditNtBalance)) {
                                            clearData();
                                            document.getElementById("lblErrMsgCancelReason").innerHTML = "Entered amount should be less than the paid amount";
                                            document.getElementById("lblErrMsgCancelReason").style.display = "";
                                            $("div.war").fadeIn(200).delay(500).fadeOut(400);
                                            ret = false;
                                        }
                            ...
                            if (ret == true) {
                                ...
                                for (var i = 1; i < addRowtable.rows.length; i++) {
                                    ...
                                    if (ret == true) {
                                        ...
                                        if (document.getElementById("tdSaleID" + P_Id).innerHTML != "" && (document.getElementById("txtPurchaseAmt" + P_Id).value != "" || document.getElementById("txtPurchaseRefSettleAmt" + P_Id).value != "")) {
                                            if (document.getElementById("tdSaleDtls" + tdLedgerRow).value == "") {
                                                document.getElementById("tdSaleDtls" + tdLedgerRow).value = document.getElementById("tdSaleID" + P_Id).innerHTML + "%" + document.getElementById("txtPurchaseAmt" + P_Id).value + "%" + document.getElementById("tdAmnt" + P_Id).innerHTML + "%" + document.getElementById("txtPurchaseRefSettleAmt" + P_Id).value + "%" + document.getElementById("tdSalesBalance" + P_Id).value;
                                            }
                                            else {
                                                document.getElementById("tdSaleDtls" + tdLedgerRow).value = document.getElementById("tdSaleDtls" + tdLedgerRow).value + "$" + document.getElementById("tdSaleID" + P_Id).innerHTML + "%" + document.getElementById("txtPurchaseAmt" + P_Id).value + "%" + document.getElementById("tdAmnt" + P_Id).innerHTML + "%" + document.getElementById("txtPurchaseRefSettleAmt" + P_Id).value + "%" + document.getElementById("tdSalesBalance" + P_Id).value;
                                            }
                                        }
                                    }
                                }
                        ...
                        function AmountCalculation(PurchaseId) {
                            ...
                            if (document.getElementById("txtPurchaseAmt" + PurchaseId).value != "") {
                                purchaseAmt = document.getElementById("txtPurchaseAmt" + PurchaseId).value;
                                purchaseAmt = purchaseAmt.replace(/\,/g, '');
                            }

                            var purchaseRefSettleAmt = 0;
                            if (document.getElementById("txtPurchaseRefSettleAmt" + PurchaseId).value != "") {
                                purchaseRefSettleAmt = document.getElementById("txtPurchaseRefSettleAmt" + PurchaseId).value;
                                purchaseRefSettleAmt = purchaseRefSettleAmt.replace(/\,/g, '');
                            }

                            var TotalSaleAmnt = parseFloat(purchaseAmt) + parseFloat(purchaseRefSettleAmt);

                            var tdSalesBalance = document.getElementById("tdSalesBalance" + PurchaseId).value;
                            tdSalesBalance = tdSalesBalance.replace(/\,/g, '');
                            var tdCreditNtBalance = document.getElementById("tdCreditBalance" + PurchaseId).value;
                            tdCreditNtBalance = tdCreditNtBalance.replace(/\,/g, '');
                            ...
                            if (parseFloat(TotalSaleAmnt) > parseFloat(tdAmnt)) {
                                clearData();
                                document.getElementById("lblErrMsgCancelReason").innerHTML = "Entered amount should be less than the sale amount";
                                document.getElementById("lblErrMsgCancelReason").style.display = "";
                                $("div.war").fadeIn(200).delay(500).fadeOut(400);
                            ...
                            if (ret == true) {
                                ...
                                for (var i = 1; i < addRowtable.rows.length; i++) {
                                    ...
                                    var purchaseRefSettleAmt = "";
                                    document.getElementById("txtPurchaseRefSettleAmt" + P_Id).style.borderColor = "";
                                    purchaseRefSettleAmt = document.getElementById("txtPurchaseRefSettleAmt" + P_Id).value;
                                    if (purchaseRefSettleAmt != "") {
                                        purchaseRefSettleAmt = purchaseRefSettleAmt.replace(/\,/g, '');
                                        TotalPurchaseAmnt = parseFloat(TotalPurchaseAmnt) + parseFloat(purchaseRefSettleAmt);
                                    }
                                ...
                                if (TotalPurchaseAmnt > TxtTotal) {
                                    ...
                                    if (document.getElementById("txtPurchaseAmt" + PurchaseId).value != "") {
                                        ...
                                        document.getElementById("lblErrMsgCancelReason").innerHTML = "Sale total amount should be less than the ledger amount";
                                        document.getElementById("lblErrMsgCancelReason").style.display = "";
                                        $("div.war").fadeIn(200).delay(500).fadeOut(400);
                                        ret = false;
                        ...
                        function loadSettleAmount(x) {
                            ...
                            var blnctamt = document.getElementById("lblBlncAmt").innerText.replace(/\,/g, '');
                            var ldgramt = document.getElementById("LedgerAmtInModalPurchse").innerText.replace(/\,/g, '');
                            ldgramt = ldgramt.split(" ");
                            ldgramt = ldgramt[0];
                            var totalamt = 0;
                            totalamt = Number(opBalance) + Number(settleamt);
                    ...
                    <table class="table table-bordered">
                        <thead class="thead1">
                            <tr>
                                ...
                                <th class="th_b8 tr_c">SALES RETURN / CC</th>
                            </tr>
                        </thead>
                        <tbody id="tableGrp">
                        </tbody>
                        <tfoot class="ft_q">
                            <tr>
                                ...
                                <th class="th_b8 tr_c tr_r am3">
                                    <input id="lblTotDeb" disabled="disabled" runat="server"  class="fgs4 tr_r"/><label id="LblDbtTot" runat="server"></label></th>
                                <th class="th_b8 tr_c tr_r am2">
                                    <input id="lblTotCrdt" disabled="disabled" runat="server" class="fgs4 tr_r" /><label id="LblCrdtTot" runat="server"></label></th>
                    ...
                    <div class="text_area_container">
                        <div class="col-md-8">
                            <div class="form-group iv">
                                ...
                                <textarea id="txtDescription" rows="4" cols="50" class="form-control" runat="server" maxlength="300" onblur="return textCounter(cphMain_txtDescription, 300)" onchage="return textCounter(cphMain_txtDescription, 300)" style="resize: none"></textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="sub_cont pull-right">
                    ...
                    <div class="save_sec">
                        ...
                        <asp:Button ID="ButtonFloat" runat="server" Style="display: none;" OnClick="btnUpdate_Click" class="btn sub2" Text="Confirm" />
  ...
  <div class="modal-dialog mod2" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h2 class="modal-title mod1 flt_l" id="exampleModalLabel"><i class="fa fa-line-chart"></i> Sales Bill of 
...
<%--      <div class="clearfix"></div>
      ...
      <div class="modal-footer">
        <div class="col-md-12">
          <div class="col-md-12">
           <label class="fg2_la1 tt_am am12 tr_r">SETTLEMENT</label></div>

      <div class="col-md-12">
        <div class="col-md-10">
        <label class="fg2_la1 tt_am am1 tr_r">Ledger Amount<span class="spn1"></span>:</label></div>
  ...
  <button id="btnImportSales" type="button" class="btn btn-success"  onclick="ButtnFillClickSales();" >Submit</button>
          ...
          <button type="button" class="btn btn-danger" data-dismiss="modal" onclick="clearData();">Cancel</button><%--evm 0044 onclick added--%>
